...
Use this Skill only for an exact `DomainDelegationTask` compiled from an admitted
`CoalitionPlan`. The task binds the organization, employee task, TaskTemplate,
requirement slots, actor-specific context projection, Worker identity, run ID,
nonce, deadline, and allowed output schemas.
...
4. Preserve the run ID, nonce, delegation-task digest, coalition-plan ref, Domain
   identity, and provider request ID when a model is called.
...
A live result is usable only when Kubernetes Worker identity and generation,
...
same frozen run. Otherwise the evidence class is `NOT_RUN` or the candidate is
rejected with zero target writes.
