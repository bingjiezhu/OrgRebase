...
This distributed collector retains the reviewed AgentTeams `v1.2.2` identity at
commit `849182af8e017168a5a200a87b1062142caf462d` in
`configs/goai-agentteams-demo.json` and `schemas/agentteams-live-run.schema.json`.
The active source in `agentteams/source-lock.json` is v1.2.3. Until a new distributed
deployment is qualified, the freezer refuses new envelopes with
`DEPLOYMENT_SOURCE_NOT_QUALIFIED`; it does not relabel the earlier deployment.
