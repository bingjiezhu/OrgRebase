...
| 20 | Skill governance scaffold | `SkillCurator`, `GovernedSkillEvaluator` | fixed action table + task/trace refs + exact candidate bytes | CANARY or QUARANTINED | 16 constructed cases across replay, held-out, negative transfer, permission, injection, malformed, resource, canary | skill candidate/evaluation receipt; no trajectory-content induction claim |
...
| Governed Skill Foundry | `CANARY` in local deterministic evaluation | fixed-action exact candidate passes 16 constructed governance cases; not trajectory learning or production gain |
