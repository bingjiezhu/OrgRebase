...
_LEGACY_CONTRACT_DIGEST = (
    "sha256:e4605d4010346671009e87266f5580d2ebb32605d7498b904ee8a8e0fa25133c"
)
_DIGEST = re.compile(r"^sha256:[0-9a-f]{64}$")
_LEGACY_INPUTS = {
    "schema": "orgrebase.impact-result.v1",
    "required": ["classification", "object_id", "reason_code", "preview_digest"],
}
_LEGACY_OUTPUTS = {
...
_LEGACY_PERMISSIONS = {
    ...
    "allowed_tools": [],
    "side_effects": [],
    "approval_requirements": ["exact_preview_approval", "human_canary_approval"],
}
...
def load_skill_contract() -> dict[str, Any]:
    ...
    if checkout.is_file():
        ...
        raw = (
            ...
            .joinpath(
                "skill_contracts/enterprise-launch-readiness-legacy-v1.3.json"
            )
...
class EnterpriseLaunchReadinessSkill:
    ...
    def __init__(self) -> None:
        ...
        if (
            self.contract_digest != _LEGACY_CONTRACT_DIGEST
            or self.contract.get("name") != "enterprise-launch-readiness"
            or self.contract.get("version") != "1.3"
            or self.contract.get("inputs") != _LEGACY_INPUTS
            or self.contract.get("outputs") != _LEGACY_OUTPUTS
            or self.contract.get("permissions") != _LEGACY_PERMISSIONS
            or self.contract.get("dependencies", {}).get("tools") != []
            or self.contract.get("multi_agent_relation", {}).get("authority")
            != "proposal only; control plane owns release state"
        ):
