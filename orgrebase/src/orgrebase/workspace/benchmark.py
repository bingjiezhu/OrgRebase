...
class OWBEvaluator:
    ...
    def evaluate_system(
        ...
        *,
        ...
        if not selected:
            raise IntegrityError("BENCHMARK_NO_CASES_SELECTED")
        ...
        for case in selected:
            ...
            if case.case_type == "FORMATION":
                ...
                target_writes = output.get("target_writes")
                security_ok = (
                    output.get("error_code") == gold.expected_error_code
                    and type(target_writes) is int
                    and target_writes == 0
                )
        ...
        core = OWBCoreScore(
            ...
            status=(
                ...
                # Correct-looking output cannot make a failed or unexecuted run
                # pass. ABSTAIN remains eligible when its output satisfies gold.
                and all(item.status in {"PASS", "ABSTAIN"} for item in receipts)
                else "FAIL"
            ),
        )
