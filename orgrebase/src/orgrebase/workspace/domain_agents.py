...
class DeterministicDomainProvider:
    def __init__(
        ...
        self.object_reader: Callable[[str, str | None], VersionedObject] | None = None

    def _lease(self, value: LocalSourceValue, now: str) -> tuple[str, str]:
        observed_at = utc_datetime(now)
        expires_at = observed_at + timedelta(seconds=900)
        valid_from = now
        if self.object_reader is not None:
            object_id, version = value.object_ref.rsplit("@", 1)
            ...
            if utc_datetime(source.valid_from) > observed_at:
                raise ValueError("SOURCE_PREMISE_NOT_EFFECTIVE")
            valid_from = source.valid_from
            if source.valid_to is not None:
                expires_at = min(expires_at, utc_datetime(source.valid_to))
            if expires_at <= observed_at:
                raise ValueError("SOURCE_PREMISE_EXPIRED")
    ...
    def source_projection(
        ...
        *,
        ...
        return ActorContextProjection(
            ...
            expires_at=min(self._lease(self._values[slot], now)[1] for slot in slot_ids),
    ...
    def produce(
        ...
        *,
        ...
        for slot_id in sorted(slot_ids):
            ...
            valid_from, fresh_until = self._lease(value, now)
