...
class ExecutionReferenceMonitor:
    ...
    def resolve(self, slot_id: str) -> JsonValue:
        ...
        if utc_datetime(binding.expires_at) <= utc_datetime(self.clock.now()):
...
class QuoteInputAssembler:
    ...
    def assemble(self, monitor: ExecutionReferenceMonitor) -> ResolvedQuoteInputs:
        ...
        declared = {slot.slot_id for slot in monitor.template.slots}
        pricing_slots = {"quote_basket", "pricing_policy"}
        if declared & pricing_slots:
            if not pricing_slots <= declared:
                raise ValueError("QUOTE_PRICING_INPUT_PAIR_REQUIRED")
            values.update({slot: monitor.resolve(slot) for slot in ("quote_basket", "pricing_policy")})
        return self.from_values(values)

    @staticmethod
    def from_values(values: Mapping[str, JsonValue]) -> ResolvedQuoteInputs:
        pricing = {}
        if "quote_basket" in values or "pricing_policy" in values:
            if not {"quote_basket", "pricing_policy"} <= values.keys():
                raise ValueError("QUOTE_PRICING_INPUT_PAIR_REQUIRED")
            pricing = {
    ...
    def render(
        ...
        *,
        ...
        return QuotePayload(
            ...
            pricing=(calculate_quote(inputs.quote_basket, inputs.pricing_policy, currency=inputs.currency)
                     if inputs.quote_basket is not None else None),
        )


def quote_output_lineage(*, priced: bool = False) -> tuple[OutputFieldLineage, ...]:
    lineage = (
    ...
    if priced:
        lineage += (OutputFieldLineage(field_path="pricing", source_kind="DETERMINISTIC_COMPUTED",
...
class RuntimeDependencyCompiler:
    version = "workspace-runtime-dependency@2.0.0"
