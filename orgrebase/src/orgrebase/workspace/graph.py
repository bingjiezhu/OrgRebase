...
_CURRENT_STATES = frozenset({
    ObjectState.CURRENT, ObjectState.ACTIVE, ObjectState.CANARY, ObjectState.REVIEW_REQUIRED,
})
...
class WorkspaceSnapshotBuilder:
    ...
    def build(
        ...
        *,
        ...
        now: str,
        version: str = "v1",
        snapshot_id: str = "workspace-graph:northstar",
        graph_namespace: str = "graph:workspace",
    ) -> WorkspaceGraphSnapshot:
        ...
        current_by_id: dict[str, list[VersionedObject]] = {}
        for item in objects:
            if item.state in _CURRENT_STATES:
                current_by_id.setdefault(item.id, []).append(item)
        target_refs: list[str] = []
        for target_id in sorted(targets):
            candidates = current_by_id.get(target_id, [])
            if len(candidates) != 1:
        ...
        revisions = {
            "graph": f"{graph_namespace}@{version}",
    ...
    def to_enterprise_fixture(
        ...
        *,
        ...
        current_refs = {
            item.ref for item in objects
            if item.state in _CURRENT_STATES
        }
        projected_edges = tuple(edge for edge in snapshot.edges if edge.consumer_ref in current_refs)
        if len({edge.id for edge in projected_edges}) != len(projected_edges):
            raise ValueError("SNAPSHOT_EDGE_ID_AMBIGUOUS")
        # Impact propagates by resource identity. Each proof still binds the exact
        # source and consumer versions; historical consumer edges stay in the snapshot.
        current_by_id = {
            object_id: object_reader(object_id, None).ref
            for object_id in dict.fromkeys(item.id for item in objects)
        }
        for edge in projected_edges:
            if edge.coverage_basis == CoverageBasis.RUNTIME_OBSERVED:
                provider = object_by_ref[edge.provider_ref]
                if provider.digest != edge.provider_digest:
                    raise ValueError(f"SNAPSHOT_RUNTIME_SOURCE_DIGEST_MISMATCH:{edge.id}")
                if current_by_id.get(provider.id) != edge.provider_ref:
                    raise ValueError(f"SNAPSHOT_RUNTIME_SOURCE_VERSION_STALE:{edge.id}")
        core_edges = tuple(
            DependencyEdge(
                id=edge.id,
                ...
                status=(EdgeStatus.ADMITTED if edge.coverage_basis in {
                    CoverageBasis.RUNTIME_OBSERVED, CoverageBasis.OWNER_DECLARED_COMPLETE,
                    CoverageBasis.CONTRACT_DECLARED, CoverageBasis.IMPORTED_VERIFIED,
                } else EdgeStatus.PROPOSED_EDGE),
                provenance_refs=(edge.source_manifest_ref, edge.source_evidence_ref,
                                 f"source-version:{edge.provider_ref}",
                                 f"consumer-version:{edge.consumer_ref}"),
            ) for edge in projected_edges
        )
        core_edge_by_id = {edge.id: edge for edge in core_edges}
        edges_by_consumer_ref: dict[str, dict[str, WorkspaceGraphEdge]] = {}
        for edge in projected_edges:
            edges_by_consumer_ref.setdefault(edge.consumer_ref, {})[edge.id] = edge
        ...
        for manifest_ref in snapshot.manifest_refs:
            if manifest_ref in imported_by_ref:
                ...
                if f"{imported.target_id}@{imported.target_version}" not in current_refs:
                    continue
                # Rebind edge IDs/fields to the committed workspace snapshot.
                target_edges = edges_by_consumer_ref.get(
                    f"{imported.target_id}@{imported.target_version}", {})
                rebound_slots = []
                for slot in imported.requirement_slots:
                    if slot.edge_id not in target_edges:
                        continue
                    edge = core_edge_by_id[slot.edge_id]
            ...
            if runtime.source_digest_scheme != "VERSIONED_OBJECT_SHA256_V1":
                raise ValueError("RUNTIME_SOURCE_DIGEST_SCHEME_UNSUPPORTED")
            target_id, target_version = split_ref(runtime.consumer_ref)
            if runtime.consumer_ref not in current_refs:
                continue
            target_edges = edges_by_consumer_ref.get(runtime.consumer_ref, {})
            slots = []
            for entry in runtime.entries:
                source_id = split_ref(entry.provider_ref)[0]
                expected_edge_id = f"edge:runtime:{sha256_digest((entry.provider_ref, entry.consumer_ref, entry.slot_id))[7:23]}"
                candidate = target_edges.get(expected_edge_id)
                if (
                    candidate is None
                    or candidate.provider_ref != entry.provider_ref
                    or candidate.provider_digest != entry.provider_digest
                    or candidate.consumer_ref != entry.consumer_ref
                    or candidate.relation != entry.relation
                    or candidate.strength != entry.strength
                    or candidate.source_manifest_ref != runtime.ref
                    or candidate.source_evidence_ref != entry.source_event_ref
                ):
                    raise ValueError(f"RUNTIME_MANIFEST_EDGE_CARDINALITY:{entry.slot_id}:0")
                edge = core_edge_by_id[candidate.id]
                slots.append(
                    DependencyRequirementSlot(
                        ...
                        strength=entry.strength,
                        coverage_basis=CoverageBasis.RUNTIME_OBSERVED,
                        provenance_refs=tuple(sorted(set(edge.provenance_refs) | {entry.source_event_ref, runtime.trace_ref})),
                    )
                )
            target_object = object_by_ref[runtime.consumer_ref]
            manifests.append(
                DependencyManifest(
