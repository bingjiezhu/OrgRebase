...
def _bundle(
    ...
    *,
    ...
    task = next(item for item in plan.tasks if item.agent_name == worker_name)
    ...
    candidate = {
        ...
        else {"ImpactCandidate": {"work:sales_quote_a": "AFFECTED_HARD"}},
        "uncertainty": [],
    ...
    path = artifact_root / worker_name / "result.json"
    ...
    evidence = {
        ...
        "artifacts": {
            "candidate_artifacts": [
                {"ref": f"result:{worker_name}", "digest": digest}
            ]
        },
    }
    live_receipt = {
        ...
        "artifacts": [
            {
                ...
                "path": f"{worker_name}/result.json",
                "digest": digest,
...
def _replace_candidate(
    ...
    path = artifact_root / str(declared["path"])
    path.write_bytes(data)
    digest = f"sha256:{hashlib.sha256(data).hexdigest()}"
...
def _valid_semantic_output(
    worker_name: str, fixture: EnterpriseFixture
) -> dict[str, object]:
    delta = build_change_set(fixture).deltas[0]
    outputs: dict[str, dict[str, object]] = {
        ...
        "legal-steward": {
            "MinimalClaimCandidate": {
                "subject": "claim:legal.customer_notice_required",
                "value": True,
                "restricted_source_text_disclosed": False,
            }
        },
        "gtm-steward": {
            "ImpactCandidate": {"work:sales_quote_a": "AFFECTED_HARD"}
        },
        "skill-curator": {
            "SkillPatchCandidate": {
                ...
                "release_ceiling": "CANARY_ONLY",
            }
        },
    }
    return outputs[worker_name]


def _ingest_role_output(
    tmp_path: Path,
    fixture: EnterpriseFixture,
    worker_name: str,
    output: dict[str, object],
):
    live_receipt, manifest, artifact_root = _bundle(
        tmp_path,
        fixture,
        bound=True,
        worker_name=worker_name,
    ...
    change_set = build_change_set(fixture)
    preview = ImpactEngine(fixture).preview(change_set)
    return AgentTeamsCandidateIngestor().ingest(
...
@pytest.mark.parametrize(
    "worker_name",
    ("product-steward", "legal-steward", "gtm-steward", "skill-curator"),
)
def test_task_specific_semantic_candidates_are_admitted(
    tmp_path: Path,
    fixture: EnterpriseFixture,
    worker_name: str,
) -> None:
    receipt = _ingest_role_output(
        tmp_path,
        fixture,
        worker_name,
        _valid_semantic_output(worker_name, fixture),
    )

    assert receipt.decisions[0].decision == "ADVISORY_ACCEPTED"
    assert receipt.decisions[0].reason_codes == ("EXACTLY_BOUND_ADVISORY",)
    assert receipt.target_writes == 0


def test_product_legacy_field_aliases_remain_semantically_bound(
    tmp_path: Path,
    fixture: EnterpriseFixture,
) -> None:
    delta = build_change_set(fixture).deltas[0]
    output = {
        "ClaimDeltaCandidate": {
            "subject": delta.object_id,
            "before": delta.base_value,
            "after": delta.proposed_value,
        }
    }

    receipt = _ingest_role_output(tmp_path, fixture, "product-steward", output)
...
@pytest.mark.parametrize(
    ...
    (
        ("object_id", "claim:product.export_format", "PRODUCT_SUBJECT_MISMATCH"),
        ("base_value", "2026-08-01", "PRODUCT_BEFORE_VALUE_MISMATCH"),
        ("proposed_value", "2026-10-01", "PRODUCT_AFTER_VALUE_MISMATCH"),
    ),
)
def test_product_semantic_drift_is_rejected_with_precise_reason(
    tmp_path: Path,
    fixture: EnterpriseFixture,
    field: str,
    value: object,
    ...
    output = _valid_semantic_output("product-steward", fixture)
    candidate = output["ClaimDeltaCandidate"]
    assert isinstance(candidate, dict)
    candidate[field] = value

    receipt = _ingest_role_output(
        tmp_path, fixture, "product-steward", output
    )

    assert receipt.decisions[0].decision == "REJECTED"
    assert receipt.decisions[0].reason_codes == (reason_code,)
...
@pytest.mark.parametrize(
    ...
    (
        (
            {"work:unscoped_dashboard": "AFFECTED_HARD"},
            "GTM_TARGET_OUT_OF_PREVIEW_SCOPE",
        ),
        ({"work:sales_quote_a": "UNKNOWN"}, "GTM_CLASSIFICATION_MISMATCH"),
    ),
)
def test_gtm_semantic_drift_is_rejected_with_precise_reason(
    tmp_path: Path,
    fixture: EnterpriseFixture,
    impact: dict[str, str],
    reason_code: str,
...
def test_legal_unrelated_claim_is_rejected_with_precise_reason(
    tmp_path: Path,
    fixture: EnterpriseFixture,
) -> None:
    output = _valid_semantic_output("legal-steward", fixture)
    candidate = output["MinimalClaimCandidate"]
    assert isinstance(candidate, dict)
    candidate["subject"] = "claim:retention_window"

    receipt = _ingest_role_output(tmp_path, fixture, "legal-steward", output)

    assert receipt.decisions[0].decision == "REJECTED"
    assert receipt.decisions[0].reason_codes == ("LEGAL_SUBJECT_MISMATCH",)


@pytest.mark.parametrize(
    ("field", "value", "reason_code"),
    (
        ("target", "skill:unrelated", "SKILL_TARGET_MISMATCH"),
        ("candidate_version", "2.0", "SKILL_VERSION_TRANSITION_MISMATCH"),
        ("release_ceiling", "GENERAL_AVAILABILITY", "SKILL_RELEASE_CEILING_MISMATCH"),
    ),
)
def test_skill_semantic_drift_is_rejected_with_precise_reason(
    tmp_path: Path,
    fixture: EnterpriseFixture,
    field: str,
    value: object,
    ...
    output = _valid_semantic_output("skill-curator", fixture)
    candidate = output["SkillPatchCandidate"]
    assert isinstance(candidate, dict)
    candidate[field] = value

    receipt = _ingest_role_output(tmp_path, fixture, "skill-curator", output)
