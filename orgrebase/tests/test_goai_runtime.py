...
def test_failure_drill_reuses_original_approval_after_apply(service: OrgRebaseService, monkeypatch) -> None:
    service.apply()
    before = service.current_view()

    def unexpected_approval(*args, **kwargs):
        raise AssertionError("cannot approve the superseded source again")

    monkeypatch.setattr(service.workflow, "approve", unexpected_approval)
    result = service.freshness_failure_drill()
    assert result["receipt"].target_writes == 0
    assert result["receipt"].expected_revisions != result["receipt"].actual_revisions
