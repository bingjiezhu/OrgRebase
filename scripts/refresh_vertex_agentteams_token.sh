...
cookie_file="${TMPDIR:-/tmp}/orgrebase-higress-session-cookie"
...
login_payload=$(jq -nc --arg username "$admin_user" --arg password "$admin_password" \
...
provider_payload=$(printf '%s' "$provider_response" | jq -ce --arg token "$vertex_token" '
  .data
  | .tokens = [$token]
...
jq -nc --arg token "$encoded_token" '{data:{AGENTTEAMS_LLM_API_KEY:$token}}' \
