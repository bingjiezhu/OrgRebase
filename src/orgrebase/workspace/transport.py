...
class WorkspaceTransportCompiler:
    ...
    def compile(
        ...
        *,
        ...
        cards = {item.ref: item for item in default_capability_cards()}
        ...
        for card_ref in plan.selected_card_refs:
            try:
                card = cards[card_ref]
                projection_ref = projection_refs_by_domain[card.domain_id]
            except KeyError as exc:
                raise IntegrityError(f"TRANSPORT_DECLARATION_MISSING:{card_ref}") from exc
            slot_ids = tuple(sorted(slots_by_domain.get(card.domain_id, ())))
            if not slot_ids or not set(slot_ids).issubset(card.supported_slot_ids):
                raise IntegrityError(f"TRANSPORT_SLOT_EXPANSION:{card.domain_id}")
            delegations.append(
                DomainDelegationTask(
    ...
    def verify(
        *,
        ...
        cards = {item.ref: item for item in default_capability_cards()}
        expected_domains = {cards[ref].domain_id for ref in plan.selected_card_refs}
        ...
        for task in delegations:
            card = next((item for item in cards.values() if item.domain_id == task.domain_id), None)
