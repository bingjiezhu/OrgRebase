...
def test_apply_rejects_unbound_agent_trace(service: OrgRebaseService) -> None:
    service.preview()
    escaped = service.collaboration["handoffs"][0].model_copy(
        update={"candidate_only": False, "digest": ""}
    )
    collaboration = {
...
def test_apply_rejects_forged_or_missing_candidate_ingestion(
    ...
    missing = {
        key: value
        for key, value in service.collaboration.items()
        if key != "candidate_ingestion"
    }
...
def test_apply_rejects_ingestion_that_rejects_a_bound_proposal(
    ...
    service.preview()
    gtm = next(
        item for item in service.collaboration["handoffs"] if item.from_agent == "gtm-steward"
    )
    ...
    handoffs = tuple(
        ...
        for item in service.collaboration["handoffs"]
    )
...
def test_content_oracle_fails_closed_on_rebuild_and_preserve_drift(
    ...
    delta = SimpleNamespace(proposed_value=sales.payload["canonical_premise"])
    before = {"work:sales_quote_a": sales, "work:legal_review_c": legal}
    ...
    with pytest.raises(IntegrityError, match="REBUILD_DID_NOT_WRITE_ADMITTED_VALUE"):
        service.workflow._assert_content_oracle(
            ...
            bounded=bounded,
            before=before,
            delta=SimpleNamespace(proposed_value="not-the-admitted-value"),
    ...
    with pytest.raises(IntegrityError, match="PRESERVE_MUTATED_BUSINESS_PAYLOAD"):
        service.workflow._assert_content_oracle(
            affected=affected,
            bounded=bounded,
            before={"work:sales_quote_a": sales, "work:legal_review_c": drifted_payload},
            delta=delta,
            transitions=[{"object_id": "work:sales_quote_a"}],
        )
    ...
    with pytest.raises(IntegrityError, match="PRESERVE_MUTATED_OBJECT_VERSION"):
        service.workflow._assert_content_oracle(
            affected=affected,
            bounded=bounded,
            before={"work:sales_quote_a": sales, "work:legal_review_c": drifted_version},
            delta=delta,
            transitions=[{"object_id": "work:sales_quote_a"}],
        )
...
def test_apply_executes_every_vmrc_disposition(service: OrgRebaseService) -> None:
    ...
    rebuilds = {
        ...
        if item.disposition == EffectDisposition.REBUILD
    }
    preserved = {
        item.target_id
        for item in certificate.effects
    ...
    requalified = {
        item.target_id
        for item in certificate.effects
        if item.disposition == EffectDisposition.REQUALIFY
    }
...
def test_approval_authority_scope_and_expiry_are_enforced(
    ...
    wrong_root = approval.model_copy(
        update={"authorization_revision": "authz:forged", "digest": ""}
    )
    ...
    expired = approval.model_copy(
        update={"expires_at": "2026-08-14T00:03:30Z", "digest": ""}
    )
