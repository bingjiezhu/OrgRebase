...
from orgrebase.domain import FreshnessError, ImpactClassification, IntegrityError


def test_launch_date_change_uses_existing_core_and_rebuilds_quote(workspace_service) -> None:
    workspace_service.form_quote()
    result = workspace_service.apply_change("launch_date")
    classifications = {
...
def test_preview_revision_drift_is_zero_write(workspace_service) -> None:
    ...
    with pytest.raises(FreshnessError):
        workspace_service.apply_change("launch_date", current_revisions=current)
...
def test_preserved_finance_payload_and_version_remain_unchanged(workspace_service) -> None:
    workspace_service.form_quote()
    before = workspace_service.store.get_object("work:finance_analysis_d")
    workspace_service.apply_change("launch_date")
