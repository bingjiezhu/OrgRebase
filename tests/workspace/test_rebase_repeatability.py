...
def test_first_rebase_updates_only_launch_date_and_promotes_graph(formed_service) -> None:
    before = formed_service.current_quote()
    finance_before = formed_service.store.get_object("work:finance_analysis_d")
    result = formed_service.apply_change("launch_date")
    after = result["quote"]
...
def test_successor_graph_contains_current_quote_manifest_and_trace(formed_service) -> None:
    launch = formed_service.apply_change("launch_date")
...
def test_rebase_extension_failure_is_zero_write(formed_service, fail_after: str) -> None:
    ...
    with pytest.raises(RuntimeError, match="INJECTED_WORKSPACE_SUCCESSOR_FAILURE"):
        formed_service.apply_change("launch_date", fail_after=fail_after)
